...
`leaderboardPosition` stays scoped to the submitting server (the nested leaderboard); `globalLeaderboardPosition` is the same shape computed across every active server for this map (the global leaderboard), added 2026-07-14 so a single response carries both without the Lua client needing a second lookup. `personalBest` groups the PB-comparison fields already implied by the top-level `bestTime`/`isNewRecord` into one object, plus two new ones: `previousTime` (this player's PB before this submission, `null` on their first-ever lap for this server+map) and `improvement` (seconds shaved off the previous PB, `null` unless `isNewRecord` is true and a previous PB existed). All three additions are purely additive — no existing top-level key changed shape or meaning, so the currently-deployed Lua client's reads (`isNewRecord`, `lapTime`, `lb.top_time`) are unaffected.
