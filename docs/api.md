...
Each entry: `rank`, `lap_id`, `player` (`id`, `name`), `server` (`id`, `name`), `time` (raw seconds), `time_formatted`, `gap` (seconds behind rank 1, `0` for rank 1 itself), `set_at` (ISO 8601), `splits` (per-checkpoint `checkpoint_id`/`duration`, 2026-07-14 — empty array for the ~96% of real laps with no split data, same shape and same sparse-coverage caveat as `GET /api/v1/laps/{lapTime}`'s own `splits` key, see [database.md](database.md)).
...
A missing map/lap (or a `?port=` claim that doesn't match any registered server) returns `404` with a JSON body: `{"message": "..."}`. The message deliberately never leaks an internal Eloquent class name — Laravel's own default for a route-model-binding failure is `"No query results for model [App\Models\Map] bloodgulch2"`, which exposes app-internal namespace/class structure to a public API consumer for no benefit. `bootstrap/app.php` maps every `ModelNotFoundException` (2026-07-14) to a clean `"No query results for {model} {id}"` instead (e.g. `"No query results for map bloodgulch2"`, `"No query results for lap time 4821"`) before Laravel's own exception handler gets a chance to render its default message — see that file's own comment for why this has to be a `map()`, not a `render()`, callback (the handler unconditionally rewrites `ModelNotFoundException` into a message-carrying `NotFoundHttpException` earlier in its `render()` flow than any registered `render()` callback is checked).
