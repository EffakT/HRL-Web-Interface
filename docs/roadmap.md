...
8. **Wire real backend data** — swap mock arrays in every Livewire component for real Eloquent queries. **In progress**: `ServerList`, `ServerShow`, `ServerMapLeaderboard`, `MapList`, and `MapLeaderboard` are wired for real. `ServerList`: real server names/laps/distinct-player counts, with mock-only region/capacity/ping removed and activity/current-map honestly derived (see [decisions.md](decisions.md)). `ServerShow`: Maps, Stats, and Latest Laps are real; Top Players stays mock, blocked on item 11. `ServerMapLeaderboard`: one best lap per player on one server+map. `MapList`: active-server global counts/bests, excluding zero-lap maps. `MapLeaderboard`: one global PB per player across active servers, deterministic ties, winning-server context, real gaps, and real split comparison. Laps belonging to soft-deleted servers are treated as nonexistent throughout global map reads. Remaining components (`PlayerList`, `PlayerShow`, `Home`) are still mock.
...
18. ~~**Wire real per-checkpoint split comparison for the Lap Detail modal**~~ — **done on every real-data consumer**: Server Single, nested Server Map Leaderboard, and global Map Leaderboard all use `LapTimeSplit::compare()`, including the #1-vs-#2 fallback. `PlayerShow` remains mock and will adopt the same helper when its lap data is wired.
