...
Real-data component coverage also lives in focused Feature files: `ServerListTest.php` checks server recency/player derivations, live-vs-proxy precedence for online status/current map, and (2026-07-06) the site-wide live-update listener; `ServerShowTest.php` checks active-only global record references; `MapListTest.php` checks active-server aggregation and zero-lap exclusion; `MapLeaderboardTest.php` checks cross-server PB ranking, ties, archived-server exclusion, links, splits, the podium split empty state, pagination/index preservation, and the live-update listener; `ServerMapLeaderboardTest.php` checks server-scoped (not global) ranking and the live-update listener; `LapTimeSplitCompareTest.php` covers every comparison branch; `ApiTest.php` covers the three read endpoints; `LapSubmissionTest.php` covers the lap-submission webhook, including (2026-07-06) that `LapSubmitted` fires on every attempt; `RefreshLiveServerInfoTest.php` covers the live-server-info scheduled command; `HomeTest.php` includes (2026-07-06) the site-wide live-update listener; `tests/Unit/LeaderboardUpdatedTest.php`/`LapSubmittedTest.php` cover the two broadcast events' channels/payloads. The suite currently passes **98 tests / 248 assertions** (2026-07-06, up from 30/80 — see below; one pre-existing pagination test is separately, temporarily failing against an in-progress, unrelated `PLAYERS_PER_PAGE` change — not part of this count's intent).
