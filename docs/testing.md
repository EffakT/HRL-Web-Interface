...
Real-data component coverage also lives in focused Feature files: `ServerListTest.php` checks server recency/player derivations, live-vs-proxy precedence for online status/current map, and (2026-07-06) the site-wide live-update listener; `ServerShowTest.php` checks active-only global record references and (2026-07-06) that Top Players links to the server-scoped player profile; `ServerPlayerShowTest.php` (2026-07-06) checks server-vs-global rank/score, Performance-by-Map/Stats isolation to one server, and the 404 guard; `MapListTest.php` checks active-server aggregation and zero-lap exclusion; `MapLeaderboardTest.php` checks cross-server PB ranking, ties, archived-server exclusion, links, splits, the podium split empty state, pagination/index preservation, and the live-update listener; `ServerMapLeaderboardTest.php` checks server-scoped (not global) ranking and the live-update listener; `LapTimeSplitCompareTest.php` covers every comparison branch; `ApiTest.php` covers the three read endpoints; `LapSubmissionTest.php` covers the lap-submission webhook, including (2026-07-06) that `LapSubmitted` fires on every attempt, (2026-07-07, SEC-01, five follow-up passes) the HRL query verification's log-only-vs-enforced behavior, idempotent replay by content-hash and by `submission_id`, reservation release on a pre-commit failure, cross-server `submission_id` non-collision, the durable `(server_id, submission_id)` DB constraint surviving a simulated cache flush, the tiered rate limiter (both tiers, a source actually upgrading mid-test, and the verified marker being revoked on a failed verification), the `servers` unique `(ip, port)` constraint (including that a soft-deleted server's old ip:port stays reusable), `409 idempotency_conflict` on a changed-content resubmission (both cache-based and, after a simulated cache flush, the durable `submission_hash` fallback), `submission_id` being required under enforcement, a reflection-based test of the unique-violation classifier methods against constructed exceptions for all three index shapes, and a duplicate-`checkpoint_id` split payload being rejected with `422`; `LapSubmissionHashTest.php` (2026-07-07, SEC-01 fourth and fifth follow-ups) covers the shared canonical content fingerprint directly — that `race_type`/`player_name` changes the hash, that splits normalize by `checkpoint_id` regardless of submission order, and that native-numeric and numeric-string variants of the same submission hash identically; `LapSubmissionVerifierTest.php` (2026-07-07, extended twice same day) covers `LapSubmissionVerifier` directly — every rejection reason, the single-retry-on-UDP-failure behavior, token-rotation grace, the tightened player-key regex, and the tri-state UDP-response handling; `RefreshLiveServerInfoTest.php` covers the live-server-info scheduled command; `HomeTest.php` includes (2026-07-06) the site-wide live-update listener; `tests/Unit/LeaderboardUpdatedTest.php`/`LapSubmittedTest.php` cover the two broadcast events' channels/payloads. The suite currently passes **139 tests / 352 assertions** (2026-07-07).

**A pre-existing coverage gap noted, not fixed**: `PlayerShow` (the global Player Single page) still has no dedicated test file of its own — only ever exercised via `RoutesTest.php`'s render-without-erroring smoke test. Its extraction into `App\Models\PlayerProfile` (2026-07-06) is covered indirectly through `ServerPlayerShowTest.php` (the second, server-scoped consumer of the same shared logic) and the full existing suite staying green through the refactor, but `PlayerShow` itself — Fav Servers, the global-scope-specific parts — remains real Livewire-component-test territory for whoever picks up the rest of item 17.

**Real-time broadcasting (roadmap item 16) is verified two different ways, neither of them a Pest test exercising the real transport** — Pest has no running WebSocket server. The listener *logic* (what a component does upon receiving the event) is a normal, fast, repeatable Pest test: call `onLeaderboardUpdated()` directly and assert the component's state re-fetched. The *transport* (Laravel → queue → Reverb → WebSocket) was verified once, manually, with a standalone `pusher-js` script bypassing Echo/Livewire entirely — see [decisions.md](decisions.md) for the exact steps and what it caught (a queue-worker requirement, a port conflict). If real browser/WebSocket testing is ever needed for this, Pest 4's browser plugin isn't currently installed (see the Rector/Semgrep tooling gaps below for the general "not everything can be installed in this sandbox" pattern) — evaluate it if broadcasting logic grows complex enough to need it.
