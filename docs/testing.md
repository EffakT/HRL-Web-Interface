...
Real-data component coverage also lives in focused Feature files: `ServerListTest.php` checks the server recency/player derivations, `MapListTest.php` checks global all-server lap aggregation plus zero-lap map exclusion, and `LapTimeSplitCompareTest.php` covers every split-comparison branch. The suite currently passes **24 tests / 45 assertions**.
