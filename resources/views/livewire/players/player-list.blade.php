<div class="mx-auto max-w-[1080px] px-6 py-14">
    ...
    <div class="mt-10">
        ...
        <div class="hidden tp:block">
            <div class="grid grid-cols-[52px_1fr_90px_90px_70px_70px_120px] gap-3 border-b border-hud-green/16 px-3.5 py-2.5 font-mono text-[9px] font-semibold tracking-[0.16em] text-hud-text-dim">
                <span>#</span><span>PLAYER</span><span class="text-right">SCORE</span><span class="text-right">RECORDS</span><span class="text-right">MAPS</span><span class="text-right">LAPS</span><span class="text-right">ACTIVE</span>
            </div>
            @foreach ($rankedPlayers as $player)
                <a href="{{ route('players.show', ['playerId' => $player['id']]) }}"
                   class="grid grid-cols-[52px_1fr_90px_90px_70px_70px_120px] items-center gap-3 border-b border-white/5 border-l-2 border-l-transparent px-3.5 py-3.5 transition hover:border-l-hud-green hover:bg-hud-green/7">
            ...
            <div class="flex items-center justify-between px-3.5 py-4 font-mono text-[10px] tracking-[0.1em] text-hud-text-dim">
                @if (count($players) > 3)
                    <span>SHOWING RANKS {{ $rankedPlayers->firstItem() + 3 }}–{{ $rankedPlayers->lastItem() + 3 }} / {{ $stats['totalPlayers'] }} PLAYERS</span>
                ...
                @else
                    <span>SHOWING {{ $stats['totalPlayers'] }} PLAYERS</span>
                @endif
            </div>
        </div>
        ...
        @if (count($players) > 3)
            <div class="flex items-center justify-between px-3.5 py-4 font-mono text-[10px] tracking-[0.1em] text-hud-text-dim tp:hidden">
                <span>RANKS {{ $rankedPlayers->firstItem() + 3 }}–{{ $rankedPlayers->lastItem() + 3 }} / {{ $stats['totalPlayers'] }}</span>
                <div class="flex items-center gap-3">
                    <button type="button" wire:click="previousPage('page')" @disabled($rankedPlayers->onFirstPage())
                            class="cursor-pointer tracking-[0.14em] disabled:cursor-not-allowed disabled:text-hud-text-faint {{ $rankedPlayers->onFirstPage() ? '' : 'text-hud-text hover:text-hud-green' }}">
                        ‹ PREV
