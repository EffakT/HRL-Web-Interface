...
<div class="mx-auto max-w-[1080px] px-6 py-14">
    ...
    <div class="mt-1.5 font-mono text-[11px] tracking-[0.05em] text-hud-text-faint">{{ $serverIp }}:{{ $serverPort }}</div>
        ...
        @if (count($players) > 3)
            <div class="mt-8 hidden tp:block">
                ...
                @foreach ($rankedPlayers as $player)
                    <a href="{{ route('servers.players.show', ['serverId' => $serverId, 'playerId' => $player['id']]) }}"
